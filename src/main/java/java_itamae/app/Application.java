...
public class Application {
  ...
  private Usage usage;
  @Inject
  @New(IsValidAttribute.class)
  private IsValidAttribute validator;
  @Inject
  @New(CreateFile.class)
  private CreateFile createFile;
  @Inject
  @New(DeleteFile.class)
  private DeleteFile deleteFile;
  @Inject
  @New(CreateDirectory.class)
  private CreateDirectory createDirectory;
  @Inject
  @New(DeleteDirectory.class)
  ...
  public int main(String[] args) {
    ...
    if (validator.test(attr)) {
      if (createFlag == 1) {
        if (fileFlag == 1) {
          status = createFile.apply(attr);
        } else if (dirFlag == 1) {
          status = createDirectory.apply(attr, recursive);
        }
      } else if (deleteFlag == 1) {
        if (fileFlag == 1) {
          status = deleteFile.apply(attr);
        } else if (dirFlag == 1) {
          status = deleteDirectory.apply(attr, recursive);
        }
      }
    } else {
      status = 1;
