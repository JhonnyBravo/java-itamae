...
public class StreamRepositoryTest {
    ...
    public static class ファイルが存在する場合 {
        ...
        public void 不正な文字エンコーディングを指定してgetReaderを実行した場合にUnsupportedEncodingExceptionが送出されること() throws Exception {
            final ContentsAttribute attr = new ContentsAttribute();
            attr.setPath("test.txt");
        ...
        @Test(expected = UnsupportedEncodingException.class)
        public void 不正な文字エンコーディングを指定してgetWriterを実行した場合にUnsupportedEncodingExceptionが送出されること() throws Exception {
            final ContentsAttribute attr = new ContentsAttribute();
            attr.setPath("test.txt");
            attr.setEncoding("NotExist");
