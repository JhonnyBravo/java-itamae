...
public class DirectoryServiceTest {
  ...
  public static class 親ディレクトリが存在しない場合 {
    ...
    @Test
    public void setRecursiveをtrueに指定した場合に親ディレクトリも含めてディレクトリが作成されて終了ステータスがtrueであること()
        throws Exception {
      ds.setRecursive(true);
      final boolean status = ds.create(attr);
      assertThat(status, is(true));
  ...
  public static class ディレクトリを削除する場合 {
    ...
    @Test
    public void
        削除対象とするディレクトリが存在する状態でsetRecursiveにtrueを指定してdeleteを実行した場合にディレクトリを一括削除できて終了ステータスがtrueであること()
            throws Exception {
